...
class pinup_price_purchase(models.Model):
    ...
    def _compute_ti(self):
        cr = self.env.cr
        cr.execute("""SELECT invoice_id FROM purchase_invoice_rel WHERE purchase_id = %d """ % (self.purchase_order_id.id))
        invoice_ids = cr.fetchall()
        tons_billing = 0
        if invoice_ids:
            for invoice_id in invoice_ids:
                for only_id in invoice_id:
                    data = self.env['account.invoice'].search([('id', '=', only_id), ('state', 'in', ['open','paid'])])
                    for line in data:
